...
class SectionalizedBusSubstation():
    ...
    def __post_init__(self):
        ...
        # left bus
        self.left_bus = self.cim.ConnectivityNode(name=f'{self.name}_left_bus', mRID=utils.new_mrid())
        self.left_bus.ConnectivityNodeContainer = self.substation
        self.network.add_to_graph(self.left_bus)
        object_builder.new_bus_bar_section(self.network, self.left_bus)

        # right bus
        self.right_bus = self.cim.ConnectivityNode(name=f'{self.name}_right_bus', mRID=utils.new_mrid())
        self.right_bus.ConnectivityNodeContainer = self.substation
        self.network.add_to_graph(self.right_bus)
        object_builder.new_bus_bar_section(self.network, self.right_bus)
    ...
    def new_bus_tie(self):
        ...
        airgap1 = object_builder.new_disconnector(self.network, self.substation, name = f'{self.substation.name}_bt1', node1 = self.left_bus, node2 = junction1)
        ...
        airgap2 = object_builder.new_disconnector(self.network, self.substation, name = f'{self.substation.name}_bt1', node1 = junction2, node2 = self.right_bus)
        airgap2.BaseVoltage = self.base_voltage
        self.network.add_to_graph(junction1)
        self.network.add_to_graph(junction2)
        
    def new_sectionalized_branch(self, series_number:int, branch_equipment:cim.ConductingEquipment, branch_terminal:cim.Terminal|int) -> None:
        ...
        airgap1 = object_builder.new_disconnector(self.network, self.substation, name = f'{self.substation.name}_{series_number+1}', node1 = self.left_bus, node2 = junction1)
        ...
        airgap3 = object_builder.new_disconnector(self.network, self.substation, name = f'{self.substation.name}_{series_number+3}', node1 = junction3, node2 = self.right_bus)
    ...
    def new_sectionalized_feeder(self, series_number:int, feeder_network:GraphModel, feeder:cim.Feeder, 
