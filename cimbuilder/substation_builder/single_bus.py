...
class SingleBusSubstation():
    ...
    def new_branch(self, series_number:int, branch_equipment:cim.ConductingEquipment, branch_terminal:cim.Terminal|int) -> None:
        ...
        junction1 = self.cim.ConnectivityNode(name=f'{self.substation.name}_{series_number}_j1', mRID = utils.new_mrid(), ConnectivityNodeContainer=self.substation)
    ...
    def new_feeder(self, series_number:int, feeder_network:GraphModel, feeder:cim.Feeder, 
        ...
        breaker = object_builder.new_breaker(self.network, container=self.substation,
                                            name = f'{self.substation.name}_{series_number}',
        ...
        airgap2 = object_builder.new_disconnector(self.network, container=self.substation,
                                                name = f'{self.substation.name}_{series_number+2}',
