...
The ``TYPE_CHECKING`` import below is the trick that lets Pylance / mypy see
``cim.Breaker``, ``cim.Terminal``, etc. without forcing a runtime dependency
on ``cimhub_2023`` specifically — that import is invisible at runtime.
...
def get_cim() -> "cim":
    ...
    against the cimhub_2023 profile while the actual module returned at
